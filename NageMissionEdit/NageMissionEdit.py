...
class 简化可视化编辑器(QWidget):
    def __init__(self):
        ...
        self.画布视图.setSceneRect(-1000, -1000, 2000, 2000)  # 扩大场景范围
        ...
        self.流程控制按钮组 = []
        流程按钮信息 = [
            ("if条件", "if(yes == 1)", QColor(255, 152, 0)),
            ("else分支", "else", QColor(255, 87, 34)),
            ("跳转标签", "goto label", QColor(156, 39, 176)),
            ("定义标签", ":label", QColor(103, 58, 183)),
            ("条件判断", "if(yes == 1)", QColor(255, 152, 0)),
        ]
        
        for 按钮名, 代码模板, 颜色 in 流程按钮信息:
            按钮 = QPushButton(按钮名)
            按钮.setMinimumHeight(35)  # 调整高度
            按钮.setStyleSheet(f"""
                QPushButton {{
                    background-color: {颜色.name()};
                    color: white;
                    border: none;
                    padding: 8px 15px;
                    font-size: 12px;
                ...
                QPushButton:hover {{
                    background-color: {颜色.lighter(120).name()};
                    border: 1px solid white;
                }}
            """)
            
            按钮.clicked.connect(lambda 选中=False, 名称=按钮名: self.创建节点(名称))
            工具栏布局.addWidget(按钮)
            self.流程控制按钮组.append(按钮)
        ...
        搜索框布局 = QHBoxLayout()
        搜索标签 = QLabel("搜索:")
        self.搜索输入框 = QLineEdit()
        self.搜索输入框.setPlaceholderText("输入命令名称搜索...")
        self.搜索输入框.textChanged.connect(lambda: self.过滤命令)
        搜索框布局.addWidget(搜索标签)
        搜索框布局.addWidget(self.搜索输入框)
        左侧布局.addLayout(搜索框布局)
        ...
        self.网格开关按钮 = QPushButton("网格开关")
        self.网格开关按钮.setCheckable(True)
        self.网格开关按钮.setChecked(True)
        self.网格开关按钮.clicked.connect(lambda: self.切换网格显示())
        ...
        # 添加到中间布局
        中间布局.addWidget(左侧面板)
        中间布局.addWidget(画布分组, 1)
        
        # 底部：状态栏
        底部状态栏 = QStatusBar()
        底部状态栏.showMessage("就绪 - 拖动命令到画布中创建节点")
    ...
    def 设置拖放功能(self):
        """设置拖放功能"""
        self.画布视图.setAcceptDrops(True)
    
    def 切换网格显示(self):
        """切换网格显示"""
        self.显示网格 = not self.显示网格
        if self.显示网格:
            self.网格开关按钮.setText("网格开")
            self.绘制网格()
        else:
            self.网格开关按钮.setText("网格关")
            self.清除网格()
    ...
    def 绘制网格(self):
        ...
        # 获取视图矩形
        视图矩形 = self.画布视图.mapToScene(self.画布视图.viewport().rect()).boundingRect()
    
        # 计算网格起始点
        start_x = int(视图矩形.left() - 视图矩形.left() % self.网格大小) - self.网格大小
        start_y = int(视图矩形.top() - 视图矩形.top() % self.网格大小) - self.网格大小
        end_x = int(视图矩形.right()) + self.网格大小
        end_y = int(视图矩形.bottom()) + self.网格大小
        ...
        x = start_x
        while x <= end_x:
            self.画布场景.addLine(x, start_y, x, end_y, grid_pen)
            x += self.网格大小
    
        # 水平线
        y = start_y
        while y <= end_y:
            self.画布场景.addLine(start_x, y, end_x, y, grid_pen)
            y += self.网格大小

    def 清除网格(self):
        """清除所有网格线"""
        for item in self.画布场景.items():
            if isinstance(item, QGraphicsLineItem):
                self.画布场景.removeItem(item)
    ...
    def eventFilter(self, obj, event):
        """事件过滤器，监听视图变化"""
        if obj == self.画布视图.viewport() and event.type() == QEvent.Paint:
            # 清除旧网格，绘制新网格
            self.更新网格()
    ...
    def 初始化命令标签页(self):
        ...
        # 所有命令按类别分组
        命令分类 = {
            "获取信息": [命令 for 命令 in 脚本命令.keys() if 命令.startswith("获取")],
            "检查条件": [命令 for 命令 in 脚本命令.keys() if 命令.startswith("检查")],
            "对话显示": [命令 for 命令 in 脚本命令.keys() if "对话" in 命令 or "问题" in 命令 or "消息" in 命令],
            "任务操作": [命令 for 命令 in 脚本命令.keys() if "任务" in 命令],
            "物品操作": [命令 for 命令 in 脚本命令.keys() if "物品" in 命令 or "金钱" in 命令 or "背包" in 命令],
            "界面操作": [命令 for 命令 in 脚本命令.keys() if "窗口" in 命令 or "按钮" in 命令],
            "其他功能": [命令 for 命令 in 脚本命令.keys() if 命令 not in 
                      [命令 for 列表 in [
                          [命令 for 命令 in 脚本命令.keys() if 命令.startswith("获取")],
                          [命令 for 命令 in 脚本命令.keys() if 命令.startswith("检查")],
                          [命令 for 命令 in 脚本命令.keys() if "对话" in 命令 or "问题" in 命令 or "消息" in 命令],
                          [命令 for 命令 in 脚本命令.keys() if "任务" in 命令],
                          [命令 for 命令 in 脚本命令.keys() if "物品" in 命令 or "金钱" in 命令 or "背包" in 命令],
                          [命令 for 命令 in 脚本命令.keys() if "窗口" in 命令 or "按钮" in 命令]
                      ] for 命令 in 列表]],
        }
    
        # 创建每个分类的标签页
        for 分类名, 命令列表 in 命令分类.items():
            if 命令列表:  # 只创建有命令的分类
                标签页 = QWidget()
                滚动区域 = QScrollArea()
                ...
                # 添加命令按钮
                for i, 命令名 in enumerate(sorted(命令列表)):
                    行 = i // 2  # 每行2个按钮
                    列 = i % 2
                ...
                # 添加拉伸
                布局.setRowStretch(行 + 1, 1)
            
                滚动区域.setWidget(容器)
                self.命令标签页.addTab(滚动区域, 分类名)
    ...
    def 添加命令按钮到布局(self, 命令列表, 布局):
        ...
        for 命令名 in 命令列表:
            if 命令名 in 脚本命令:
                按钮 = QPushButton(命令名)
                按钮.setMinimumHeight(45)  # 增大高度
                按钮.setMaximumWidth(320)  # 增大宽度
    ...
    def 创建命令节点(self, 命令名):
        ...
        # 设置位置（避免重叠，考虑缩放）
        self.节点计数器 += 1
        x = 50 + (self.节点计数器 % 5) * (节点.宽度 + 60)  # 增加间距
        y = 50 + (self.节点计数器 // 5) * (节点.高度 + 80)  # 增加间距
    ...
    def 放大画布(self):
        ...
        self.网格大小 = int(self.网格大小 * 1.2)
        self.更新网格()
    
    def 缩小画布(self):
        """缩小画布"""
        self.画布视图.scale(0.8, 0.8)
        self.网格大小 = max(10, int(self.网格大小 * 0.8))
        self.更新网格()
    ...
    def 创建节点(self, 节点名称):
        ...
        if 节点名称 in 脚本命令:
            # 创建命令节点
            节点 = 可视化命令节点(节点名称)
        elif 节点名称 in ["if条件", "else分支", "跳转标签", "定义标签", "代码块开始", "代码块结束", "条件判断"]:
            # 创建流程控制节点
            节点 = 可视化流程节点(节点名称)
        else:
            return
        ...
        # 设置位置（避免重叠）
        self.节点计数器 += 1
        x = 50 + (self.节点计数器 % 5) * (节点.宽度 + 40)  # 调整间距
        y = 50 + (self.节点计数器 // 5) * (节点.高度 + 50)  # 调整间距
    ...
    def 过滤命令(self, 搜索文本):
        """根据搜索文本过滤命令"""
        搜索文本 = 搜索文本.lower().strip()
    
        # 遍历所有标签页
        for 索引 in range(self.命令标签页.count()):
            滚动区域 = self.命令标签页.widget(索引)
            容器 = 滚动区域.widget()
            ...
            if 容器:
                ...
                if 布局:
                    ...
                    for i in range(布局.count()):
                        部件 = 布局.itemAt(i).widget()
                        if isinstance(部件, QPushButton):
                            按钮文本 = 部件.text().lower()
                            if 搜索文本 and 搜索文本 not in 按钮文本:
                                部件.hide()
                            else:
                                部件.show()
...
class 可视化命令节点(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawText(QRectF(0, 15, self.宽度, 30), Qt.AlignCenter, self.命令名)
        
        # 参数类型 - 增大字体
        命令类型 = self.定义[0]
        painter.setFont(QFont("宋体", 10, QFont.Bold))
        painter.setPen(QColor(240, 240, 240))  # 浅灰色
        painter.drawText(QRectF(0, 45, self.宽度, 25), Qt.AlignCenter, f"参数: {命令类型}")
...
class 可视化流程节点(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        }.get(self.流程名, QColor(158, 158, 158))
        ...
        painter.drawText(QRectF(0, 25, self.宽度, 40), Qt.AlignCenter, self.流程名)
